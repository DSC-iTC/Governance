= Dedicated Security Components iTC Interpretation Team (DIT) Membership
:showtitle:
:sectnumlevels: 3
:table-caption: Table
:imagesdir: images
...
=== Initial Membership
The initial membership of the {iTC-ITname} would be determined by the general iTC. The iTC will first select/approve the "officers" of the {iTC-ITname} (two or three, depending on the iTC needs). Selection of the initial officers is by the normal process defined by the ToR. 
